...
const userNavigation = [
  ...
  // Set the selected language based on the current language in i18n or default to the first language in the list
  const [selected, setSelected] = useState(() => {
    const currentLang = languages.find((l) => l.code === i18n.language);
    return currentLang ? currentLang : languages[0];
  });

  // Effect to change the language whenever `selected` changes
  useEffect(() => {
          ...
          {/* <div className="hidden md:block">
            <div className="ml-4 flex items-center md:ml-6"> */}
              <select
                ...
                className="rounded-md bg-white border border-gray-300 px-2 py-1 text-sm focus:outline-none"
              >
                {languages.map((lang) => (
